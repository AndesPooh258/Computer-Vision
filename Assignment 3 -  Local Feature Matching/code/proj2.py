...
if __name__ == '__main__':
    ...
    test_case = 'Notre Dame'
    image1 = cv2.imread('../data/' + test_case + '/img1.jpg').astype('single')
    image2 = cv2.imread('../data/' + test_case + '/img2.jpg').astype('single')
    eval_file = '../data/' + test_case + '/img1_to_img2.npy'
    ...
    x1, y1 = np.squeeze(x1).astype(int), np.squeeze(y1).astype(int)
    x2, y2 = np.squeeze(x2).astype(int), np.squeeze(y2).astype(int)
    ...
    matches, confidences = mf.match_features(image1_features, image2_features, threshold=1.4)
