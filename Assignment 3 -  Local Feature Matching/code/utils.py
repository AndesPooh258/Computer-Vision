...
def cheat_interest_points(eval_file, scale_factor):
    ...
    """
    data = np.load(eval_file, encoding='latin1', allow_pickle=True).tolist()
    x1 = data['x1']
    y1 = data['y1']
    x2 = data['x2']
    y2 = data['y2']
...
def evaluate_correspondence(imgA, imgB, ground_truth_correspondence_file, scale_factor, x1_est, y1_est, x2_est, y2_est, file_name='result'):
    ...
    data = np.load(ground_truth_correspondence_file, encoding='latin1', allow_pickle=True).tolist()
    ...
    for i in range(x1_est.shape[0]):
        ...
        most_similar_offset = [(x1[best_matches[0]] - x2[best_matches[0]]).item(), (y1[best_matches[0]] - y2[best_matches[0]]).item()]
