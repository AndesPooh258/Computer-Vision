...
def Train():
   ...
   transform_train = transforms.Compose([
               ...
               RandomCrop(size=32),
               RandomFlip(),
               # TODO: uncomment these lines for task 4
               Cutout(),
   ...
   ##### cosine annealing decay (for task 4 only)
   scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)

   ##### lr multi step schedule (for task 3 only)
   ...
   for epoch in range(start_epoch, end_epoch):
      # adjust learning rate at the beginning of every epoch (for task 3 only)
      ...
      log(args, 'Train acc_n %.5f lr %.5f' % (acc_n / input.size(0), get_lr(optimizer)))

      # adjust learning rate at the end of every epoch (for task 4 only)
      scheduler.step()
